...
// Define exam and question types
...
async function geminiApi({ mode, examName }: { mode: 'exam' | 'full', examName: string }): Promise<{ mcq: MCQ[]; frq: FRQ[] }> {
  if (mode === 'exam') {
    // Create mock exam with 5 MCQs and 1 FRQ for demo
    const mcq = Array.from({ length: 5 }).map((_, i): MCQ => ({
  ...
  if (mode === 'full') {
    ...
    const mcq = Array.from({ length: 55 }).map((_, i): MCQ => ({
      question: `${examName} — Full-Length MCQ ${i + 1}`,
      choices: ["A", "B", "C", "D"],
      answer: ["A", "B", "C", "D"][i % 4]
    }));
    const frq = Array.from({ length: 3 }).map((_, i): FRQ => ({
...
const ModernStudyApp: React.FC = () => {
  ...
  function go(r: typeof route) { setRoute(r); }

  async function generateExam(examName: string) {
    setExamError(null);
    setExamLoading(true);
    setExamData(null);
    setSelectedExam(examName);
    try {
      const body = await geminiApi({ mode: 'exam', examName });
  ...
  async function generateFullExam(examName: string) {
    setExamError(null);
    setExamLoading(true);
    setExamData(null);
    setSelectedExam(examName);
    try {
      const body = await geminiApi({ mode: 'full', examName });
      if (!body || !Array.isArray(body.mcq) || !Array.isArray(body.frq)) throw new Error('Bad exam format');
              ...
              {AP_EXAMS.map((exam) => (
                <div key={exam} className="bg-white p-4 rounded shadow flex flex-col">
                  ...
                  <div className="mt-4 flex gap-2 flex-wrap">
                    ...
                    <button onClick={() => generateFullExam(exam)} className="px-3 py-1 bg-teal-600 text-white rounded">Take Full-Length Exam</button>
                  </div>
                </div>
              ))}
            </div>
...
const ExamSession: React.FC<{ exam: ExamData; onClose: () => void }> = ({ exam, onClose }) => {
  const [index, setIndex] = useState(0);
  const [mcqAnswers, setMcqAnswers] = useState<(string | null)[]>(Array(exam.mcq.length).fill(null));
  const [frqAnswers, setFrqAnswers] = useState<string[]>(Array(exam.frq.length).fill(''));

  const total = exam.mcq.length + exam.frq.length;

  function jumpTo(i: number) { if (i >= 0 && i < total) setIndex(i); }
  function handleMcqAnswer(choice: string) { const copy = [...mcqAnswers]; copy[index] = choice; setMcqAnswers(copy); }
  function handleFrqChange(text: string) {
    const idx = index - exam.mcq.length;
    if (idx >= 0) {
      const copy = [...frqAnswers];
      copy[idx] = text;
      setFrqAnswers(copy);
    }
  }
